...
copyGUID:SetCallback(function()
	...
	MessageBox.Show(
		"Success",
		("%s's GUID was copied to your clipboard."):format(selectedInstance.Name),
		MessageType.OK
	)
end)
...
copyProtoNameContext:SetCallback(function()
	if selected.selectedProto and selected.selectedProto.name ~= "" then
		...
		MessageBox.Show(
			"Success",
			("%s was copied to your clipboard."):format(selected.selectedProto.name),
			MessageType.OK
		)
	end
end)
...
copyProtoInfoContext:SetCallback(function()
	if selected.selectedProto then
		...
		MessageBox.Show(
			"Success",
			("A table with %s's information was copied to your clipboard."):format(
...
copyConstantValue:SetCallback(function()
	if selected.selectedConstant then
		...
		MessageBox.Show(
			"Success",
			("The value of the constant %d was copied to your clipboard."):format(selected.selectedConstant.index),
			MessageType.OK
		)
	end
end)
...
local function createProto(index, value, Instance)
	...
	ListButton.new(instance, protosList):SetRightCallback(function()
		selected.selectedProto =
			{ index = index, value = value, Instance = Instance, name = toString(getInfo(value).name or "") }
